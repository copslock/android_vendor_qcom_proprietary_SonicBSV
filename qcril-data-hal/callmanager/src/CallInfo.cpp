...
bool CallInfo::setProfileInfo(CallSetupProfile *profile) {
  ...
  if((dsiHandle = dsi_get_data_srvc_hndl(CallManager::dsiNetCallback, this)) == nullptr) {
    ...
    if(pProfileInfo->lookup()) {
      if(mCallBringupCapability & BRING_UP_APN_TYPE) {
        ...
        if(pProfileInfo->getCdmaProfileId() == QDP_PROFILE_ID_INVALID || pProfileInfo->getUmtsProfileId() == QDP_PROFILE_ID_INVALID) {
          ...
          apnInfo.buf_val = (char *)pProfileInfo->getApn().c_str();
          apnInfo.num_val = pProfileInfo->getApn().length();
...
void CallInfo::handleHandoverTimeout() {
  ...
  event.callEventData = (CallInfo *)this;
  Log::getInstance().d("[CallInfo]: apn = "+getProfileInfo()->getApn());
  int nextState = getCallStateMachine()->processEvent(event);
  getCallStateMachine()->setCurrentState(nextState);
...
int CallInfo::handleRilEventDataCallback(std::shared_ptr<Message> msg) {
  ...
  int nextState = pCallInstance->getCallStateMachine()->processEvent(event);
  pCallInstance->getCallStateMachine()->setCurrentState(nextState);
