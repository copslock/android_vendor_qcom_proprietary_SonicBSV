    ...
    for (unsigned int i=0 ; i<count ; i++) {
      ...
      if (addr_info[i].iface_addr_s.valid_addr) {
        if (addr_info[i].iface_addr_s.addr.ss_family == AF_INET) {
          AddressFormatIPv4WithSubnetMask(addr_info[i].iface_addr_s.addr.__data,
            addr_info[i].iface_mask, tempStr, sizeof(tempStr));
          v4Connected = true;
        }
        else if (addr_info[i].iface_addr_s.addr.ss_family == AF_INET6) {
          AddressFormatIPv6WithPrefixLength(addr_info[i].iface_addr_s.addr.__data,
            addr_info[i].iface_mask, tempStr, sizeof(tempStr));
          v6Connected = true;
        }
        else {
      ...
      if (addr_info[i].gtwy_addr_s.valid_addr) {
        if (addr_info[i].gtwy_addr_s.addr.ss_family == AF_INET) {
          AddressFormatIPv4(addr_info[i].gtwy_addr_s.addr.__data, tempStr, sizeof(tempStr));
        }
        else if (addr_info[i].gtwy_addr_s.addr.ss_family == AF_INET6) {
          AddressFormatIPv6(addr_info[i].gtwy_addr_s.addr.__data, tempStr, sizeof(tempStr));
        }
        else {
      ...
      if (addr_info[i].dnsp_addr_s.valid_addr) {
        if (addr_info[i].dnsp_addr_s.addr.ss_family == AF_INET) {
          AddressFormatIPv4(addr_info[i].dnsp_addr_s.addr.__data, tempStr, sizeof(tempStr));
        }
        else if (addr_info[i].dnsp_addr_s.addr.ss_family == AF_INET6) {
          AddressFormatIPv6(addr_info[i].dnsp_addr_s.addr.__data, tempStr, sizeof(tempStr));
        }
        else {
        ...
        if (addr_info[i].dnss_addr_s.valid_addr) {
          ...
          if (addr_info[i].dnss_addr_s.addr.ss_family == AF_INET) {
            AddressFormatIPv4(addr_info[i].dnss_addr_s.addr.__data, tempStr, sizeof(tempStr));
          }
          else if (addr_info[i].dnss_addr_s.addr.ss_family == AF_INET6) {
            AddressFormatIPv6(addr_info[i].dnss_addr_s.addr.__data, tempStr, sizeof(tempStr));
          }
          else {
