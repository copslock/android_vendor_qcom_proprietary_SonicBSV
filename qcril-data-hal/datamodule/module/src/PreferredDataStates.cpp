...
void ApTriggeredState::enter() {
    if (mStateInfo->isRilIpcNotifier) {
        ...
        if (pendingMessage != nullptr) {
            ...
            if(mStateInfo->mVoiceCallInfo.voiceSubId != INVALID_VOICE_SUB_ID  && mStateInfo->mVoiceCallInfo.voiceSubId != mStateInfo->dds && mStateInfo->mVoiceCallInfo.voiceCallState !=  VoiceCallStateEnum::CALL_STATE_END)
            {
                mStateInfo->switch_type = DSD_DDS_SWITCH_TEMPORARY_V01;
                Log::getInstance().d("Temporary DDS switch on sub" + std::to_string(pendingMessage->getModemId()));

            }
            else
            {
                mStateInfo->switch_type = DSD_DDS_SWITCH_PERMANENT_V01;
                Log::getInstance().d("Permanent DDS switch on sub" + std::to_string(pendingMessage->getModemId()));
            }
    ...
    switch (e.event) {
        case CurrentDDSInd:
        {
            if (mStateInfo->isRilIpcNotifier) {
                ...
                sub_info.dds_sub_id = switchInfo->subId;
                if(mStateInfo->mVoiceCallInfo.voiceSubId != mStateInfo->dds && mStateInfo->mVoiceCallInfo.voiceCallState !=  VoiceCallStateEnum::CALL_STATE_END)
                {
                  sub_info.switch_type = DSD_DDS_DURATION_TEMPORARY_V01;
                }
                else
                {
                  sub_info.switch_type = DSD_DDS_DURATION_PERMANANT_V01;
                }
                SM_LOGD("[ApTriggeredState]: Sending response to qcril common");
                auto msg = std::make_shared<rildata::NewDDSInfoMessage>(global_instance_id, sub_info);
                msg->broadcast();
            }
            return HANDLED;
        }
