...
void NetworkAvailabilityHandler::processQmiDsdSystemStatusInd(dsd_system_status_ind_msg_v01 *ind_data)
{
  Log::getInstance().d("[NetworkAvailabilityHandler]: process system status ind");
  logBuffer.addLogWithTimestamp("process system status ind, apn_avail_sys valid=" +
    std::to_string((int)ind_data->apn_avail_sys_info_valid) + ", len=" + std::to_string((int)ind_data->apn_avail_sys_info_len));
  updateNetworkAvailabilityCache(ind_data->apn_avail_sys_info, ind_data->apn_avail_sys_info_len);
  dumpCache();
